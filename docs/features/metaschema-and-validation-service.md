...
- [`validation-service.md`](./validation-service.md) — the runtime container: Node/Express service, `POST /api/validate`, schema publication URLs, the merged-artifact static URLs, the `arch-validate` dev CLI, `USAGE.md` at the container root, Prometheus, Helm deploy. **Status: planned; not yet started.**
