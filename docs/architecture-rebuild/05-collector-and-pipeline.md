...
3. **Reconcile the capability enum + backfill nodes.** Every `Capability` id referenced from any artifact must exist in `schema/v0.1/enums/capabilities.yaml`. Unknown reference = build failure (a producer must PR the enum first). The enum is the capability *catalogue*: producers reference `cap:` ids but don't own them, so the collector then materialises a Capability node from the enum entry for every capability that is referenced but no producer declares (one shared node per capability, stamped `producer: schema`, only for referenced ones, skipping any a producer already declares). Without this, a `… —Realization→ cap:iam` edge would dangle on a node nobody emitted.
...
6. **Cross-reference resolution.** A single `ResolutionIndex` over the merged set offers three lookups: UUID (the UUID portion of any composite id), full string (catalogue / curated kebab ids), and per-producer hint (instance-kind hint-only refs, internal-only). Every relation's `source` and `target` id must resolve. Dangling = build failure. Reference to a `deprecated` element = warning. Reference to a `removed` element = build failure. The opt-in `--relaxed` flag downgrades *dangling* to a report warning (`removed` stays a failure) so a partially-onboarded federation builds while a referenced producer is still offline; it resolves nothing extra, so refs self-heal as their producers come online. Strict is the default; drop `--relaxed` from the pipeline once every referenced producer emits.
...
| Capability referenced but no producer declares it | Node backfilled from the enum (one shared `producer: schema` node). |
| Duplicate id across producers | Build fails. |
| Dangling cross-producer id reference | Build fails (warning under `--relaxed`). |
