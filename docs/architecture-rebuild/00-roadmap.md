...
The "pending v0.1 tightening" from `../features/metaschema-design.md` landed during this phase: instance-kind ids are now composite (`<kind>:<hint>,<uuid4>`), `producer:` and `replacedBy:` fields plus all lifecycle conditional rules are gone, and reference divergence is detected at merge time by comparing the hint portion of every composite reference against the owner's declared hint.
