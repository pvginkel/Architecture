...
# reads the JSON arrays through this map. emit_vocab_ts() asserts it stays in
# lockstep with subset.yaml's kinds — a drift fails the generator loudly.
KIND_TO_ARRAY: dict[str, str] = {
    "Node": "nodes",
    "Device": "devices",
...
# ---------- viewer vocab emission ----------


def humanise(name: str) -> str:
    """Turn a PascalCase / kebab / snake identifier into a display label.

    `ApplicationComponent` -> "Application Component";
    `cross-cutting` -> "Cross Cutting"; `AndJunction` -> "And Junction".
    """
...
def _ts_union_array(name: str, type_name: str, values: list[str]) -> str:
...
def _ts_record(name: str, key_type: str, value_type: str, mapping: dict[str, str]) -> str:
    body = "".join(f"  {json.dumps(k)}: {json.dumps(v)},\n" for k, v in mapping.items())
    return (
        f"export const {name} = {{\n{body}}} as const satisfies "
...
def emit_vocab_ts(subset: Mapping[str, Any], relationship_types: set[str]) -> str:
    ...
    types are sorted (same set the relations schema is built from); enum-backed
    vocab keeps enum-file order. Layer ids are derived from the kinds in
    first-appearance order.
    """
    kinds = subset["kinds"]
    kind_names = list(kinds.keys())
    if set(KIND_TO_ARRAY) != set(kind_names):
        raise ValueError(
            "KIND_TO_ARRAY in generate.py has drifted from subset.yaml kinds: "
            f"{sorted(set(KIND_TO_ARRAY) ^ set(kind_names))}"
        )

    layer_for = {k: kinds[k]["layer"] for k in kind_names}
    layer_ids: list[str] = []
    for k in kind_names:
        if layer_for[k] not in layer_ids:
            layer_ids.append(layer_for[k])

    rel_types = sorted(relationship_types)

    caps = load_yaml(SCHEMA_DIR / "enums" / "capabilities.yaml")["entries"]
    lifecycles = load_yaml(SCHEMA_DIR / "enums" / "lifecycle-states.yaml")["entries"]
    environments = load_yaml(SCHEMA_DIR / "enums" / "environments.yaml")["entries"]

    out: list[str] = ["// GENERATED by tooling/generate.py — do not edit\n\n"]
    out.append(_ts_union_array("ELEMENT_KINDS", "ElementKind", kind_names))
    out.append(_ts_union_array("LAYER_IDS", "LayerId", layer_ids))
    out.append(_ts_record("KIND_TO_ARRAY", "ElementKind", "string", dict(KIND_TO_ARRAY)))
    out.append(_ts_record("KIND_TO_LAYER", "ElementKind", "LayerId", layer_for))
    out.append(_ts_union_array("RELATIONSHIP_TYPES", "RelationshipType", rel_types))
    ...
    if check:
        ...
        if GENERATED_DIR.exists():
            for orphan in GENERATED_DIR.iterdir():
                if orphan not in new_files:
                    ...
                    drift = True
        existing_vocab = VOCAB_TS_PATH.read_text() if VOCAB_TS_PATH.exists() else None
        if existing_vocab != vocab_content:
            click.echo(f"DRIFT: {VOCAB_TS_PATH.relative_to(REPO_ROOT)}", err=True)
            drift = True
        if drift:
            sys.exit(1)
        ...
        for orphan in GENERATED_DIR.iterdir():
            if orphan not in new_files:
                ...
                orphan.unlink()
        VOCAB_TS_PATH.parent.mkdir(parents=True, exist_ok=True)
        VOCAB_TS_PATH.write_text(vocab_content)
        click.echo(f"Wrote {len(new_files)} files to {GENERATED_DIR.relative_to(REPO_ROOT)}.")
        click.echo(f"Wrote {VOCAB_TS_PATH.relative_to(REPO_ROOT)}.")
