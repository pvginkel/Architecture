...
def check_views(
    ...
    for view in views:
        ...
        producer_vocab, producer_source = field_vocab["producers"]
        for value in view.get("excludeProducers") or []:
            if value not in producer_vocab:
                messages.append(
                    f"view {vid!r}: excludeProducers value {value!r} "
                    f"not in {producer_source}"
                )
